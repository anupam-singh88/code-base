...
export class JobsController {
  ...
  // ParseIntPipe by default throws a BadRequestException with status code 400 if the value cannot be parsed to an integer.
  @Get(":id")
  ...
  findJobDetails(
    @Param(
      "id",
      new ParseIntPipe({ errorHttpStatusCode: HttpStatus.NOT_ACCEPTABLE }) // for customization of error message status code
    )
    id: number
  ) {
    return this.jobsService.findById(id);
  ...
  setUpdateJobExp(
    @Param("id", ParseIntPipe) id: number,
    @Query("exp",
      new DefaultValuePipe(1) // set a default value
      , ParseIntPipe) exp: number
  ) {
